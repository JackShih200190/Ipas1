...
```
```
1. John is analyzing(分析) strange(奇怪) behavior on computers in his network. He believes there is malware(惡意軟體)
on the machines. The symptoms include strange behavior that persists(堅持), even if he
boots the machine to a Linux Live CD. What is the most likely cause?
A. Ransomware(勒索軟體)
B. Boot sector virus(開機形病毒)
C. Rootkit
D. Key logger(鍵盤側錄)
Ans:B

2. Ahmed is a sales manager with a major(重大) insurance(保險) company. He has received(收到) an email that
is encouraging(鼓勵) him to click on a link and fill out a survey. He is suspicious(可疑) of the email,
but it does mention(提到) a major insurance association(協會), and that makes him think it might be
legitimate(合法). Which of the following best describes this attack?
A. Phishing
...
software on your bank’s database server that is not supposed(應該) to be there. It appears that
...
4. You are responsible(負責任的) for incident response(事件響應) at Acme bank. The Acme bank website has been
attacked. The attacker used the login screen, but rather than enter login credentials(登入信息), he or
...
are complaining(抱怨) of slow connectivity. When she examines the firewall logs, she observes a
...
6. Frank is deeply concerned(深深關注) about attacks to his company’s e-commerce server. He is particularly(尤其)
...
7. You are responsible(負責任的) for network security at Acme Company. 
Users have been reporting that personal data is being stolen(被盜) when using the wireless(無線) network. They all insist(堅持) they
only connect to the corporate(企業的) wireless access point (WAP). However, logs for the WAP
show that these users have not connected to it. Which of the following could best explain
this situation?
A. Session hijacking
B. Clickjacking(點擊搶劫)
C. Rogue access point(惡意無線基地台)
D. Bluejacking(藍芽劫持)
Ans:C

8. What type of attack depends(攻擊取於) on the attacker entering JavaScript into a text area that is intended for users to enter text that will be viewed by other users?
A. SQL injection
B. Clickjacking
...
10. Your company outsourced(外包) development of an accounting application(開發會計應用程序) to a local programming
firm. After three months of using the product, one of your accountants accidently(不小心)
...
11. Teresa is the security manager for a mid-sized insurance company. She receives(收到) a call
from law enforcement(強制執行), telling her that some computers on her network participated(參與了) in a
massive(龐大的) denial-of-service (DoS) attack. Teresa is certain(某些) that none of the employees at her
company would be involved in(捲入) a cybercrime(網路犯罪). What would best explain this scenario?
...
12. Mike is a network administrator with a small financial(金融) services company. He has received
a popup window(彈出視窗) that states(狀態) his files are now encrypted(加密的) and he must pay .5 bitcoins to get
...
13. Terrance is examining(檢查) logs for the company e-commerce web server. He discovers a number
of redirects(重新導向) that cannot be explained(解釋). After carefully(小心的) examining the website, he finds
some attacker performed(已執行) a watering hole attack by placing JavaScript in the website and is
redirecting users to a phishing website. Which of the following techniques(技術) would be best
...
actually hold?
A. Bluesnarfing
B. Buffer overflow(緩衝區溢位攻擊)
C. Bluejacking
D. DDoS
Ans:D

15. You have been asked to test your company network for security issues(問題). The specific test
you are conducting(執行) involves primarily(主要涉及) using automated(自動化的) and semiautomated(半自動化的) tools to look
for known vulnerabilities(漏洞) with the various systems on your network. Which of the following
best describes this type of test?
A. Vulnerability scan(漏洞掃描器)
B. Penetration test(滲透測試)
C. Security audit(資訊科技安全審計)
D. Security test
Ans:A

16. Jared discovers that attackers have breached(違反) his WiFi network. They have gained(獲得) access
via(通過) the wireless access point (WAP) administrative panel, and have logged on with the
credentials(證書) the WAP shipped with(附帶). What best describes this issue?
A. Default configuration()
B. Race conditions(競爭危害)
C. Failure to patch(修補失敗)
D. Weak encryption(弱加密)
Ans:D

17. Joanne is concerned(關心) about social engineering. She is particularly(尤其) concerned that this technique could be used by an attacker to obtain(獲取) information about the network, including
possibly even passwords. What countermeasure(對策) would be most effective(有效) in combating
social engineering?
A. SPI firewall
B. An IPS
C. User training
...
19. Coleen is the web security administrator for an online auction(拍賣) website. A small number
...
explain this?
A. Typosquatting(誤植域名)
B. SQL injection
C. Cross-site scripting
D. Cross-site request forgery
Ans:A

20. Mahmoud is responsible for managing security at a large university. He has just performed(已執行)
a threat(威脅) analysis for the network, and based on(基於) past incidents(過去的事件) and studies of
similar networks, he has determined(堅決) that the most prevalent threat(普遍威脅) to his network is
low-skilled attackers who wish to breach the system, simply to(只是為了) prove(證明) they can or for
some low-level crime, such as changing a grade. Which term(術語) best describes(描述) this type of
attacker?
A. Hacktivist(激進駭客)
B. Amateur(業餘)
C. Insider(內幕)
D. Script kiddie(駭客新手)
Ans:D

21. Which of the following best describes(描述) a collection of computers that have been compromised(妥協的)
...
22. John is conducting(進行) a penetration test(滲透測試) of a client’s network. He is currently(目前) gathering(蒐集) information
...
24. What type of attack is based on(基於) entering fake entries into a target networks domain name
server?
A. DNS poisoning
B. ARP poisoning
C. Bluesnarfing
...
26. You work for a security company that performs penetration testing(滲透測試) for clients. You are
